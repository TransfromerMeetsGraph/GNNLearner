...
class OneModelConfig(FairseqDataclass):
    ...
    quant_noise_pq: float = field(
        default=0.0, metadata={"help": "iterative PQ quantization noise at training time"},
    )
    quant_noise_pq_block_size: int = field(
        default=8, metadata={"help": "block size of quantization noise at training time"},
    )
...
class GraphHubInterface(nn.Module):
    ...
    def inference(
        ...
        preds0 = []
        preds1 = []
        for i in tqdm(range(0, total, bsz), total=int((total + bsz - 1) / bsz)):
            data_list = []
            ...
            preds0.append(pred[0].argmax(dim=-1).detach().cpu())
            preds1.append(pred[1].detach().cpu())
            
        preds0 = torch.cat(preds0, dim=0)
        # preds0 = preds0.argmax(dim=-1)
        preds0 = preds0 + self.task.label_min
        preds0 = preds0 * gold_num

        preds1 = torch.cat(preds1, dim=0).view(-1)
        preds1 = self.task.inverse_transform(preds1)

        preds = (preds0 + preds1) / 2
        return preds0, preds1, preds
