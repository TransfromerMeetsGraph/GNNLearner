...
from fairseq.models.gnn_kdd.gnn import GNN
...
class DoubleModelConfig(FairseqDataclass):
    ...
    gnn_virtual_node: bool = field(default=True)
    gnn_residual: bool = field(default=True)
    gnn_dropout: float = field(default=0.1)
    gnn_graph_pooling: str = field(default="mean")
    # not used
    ...
    bottleneck_ratio: float = field(default=4)
    bottleneck_layer: int = field(default=2)

    same_backbone: bool = field(default=False)


@register_model("doublemodel", dataclass=DoubleModelConfig)
class DoubleModel(BaseFairseqModel):
    def __init__(self, args, encoder0, encoder1):
        ...
        self.build_heads()
        # self.encoder0_ison = self.args.coeff0 > 0 and self.args.datatype in ["tt"]
        # self.encoder1_ison = self.args.coeff1 > 0 and self.args.datatype in ["gg"]
    ...
    def build_model(cls, args, task):
        if args.datatype == "tt":
            ...
            encoder0 = GNN(
                num_layers=args.gnn_number_layer,
                emb_dim=args.gnn_embed_dim,
                gnn_type=args.gnn_type,
                virtual_node=args.gnn_virtual_node,
                residual=args.gnn_residual,
                drop_ratio=args.gnn_dropout,
                graph_pooling=args.gnn_graph_pooling
            )
            if args.same_backbone:
                encoder1 = encoder0
            else:
    ...
    def forward(self,
        ...
        if classification_head_name is not None:
            features_only = True
        if self.args.datatype == "tt":
            x0, pred0 = self.encoder0(**net_input0, features_only=features_only, **kwargs)    
            x1, pred1 = self.encoder1(**net_input0, features_only=features_only, **kwargs)
        elif self.args.datatype == "gg":
            x0 = self.encoder0(net_input1["graph"])    
            x1 = self.encoder1(net_input1["graph"])
        else:
        ...
        if self.args.datatype == "tt":
            x0 = self.get_cls(x0)
            x1 = self.get_cls(x1)
        x_c = self.classification_heads["tc"](x0)
        x_r = self.classification_heads["tr"](x1)
        # return x_c, x_r
        ...
        output_dict = {}
        output_dict['pred0'] = x0
        output_dict['pred1'] = x1
        if ret_contrastive:
    ...
    def register_classification_head(self, name, num_classes=None, inner_dim=None, encoder="t", **kwargs):
        ...
        # )
        input_dim = self.args.encoder_embed_dim if encoder == "t" else self.encoder0.out_dim
        if inner_dim is None:
            inner_dim = self.args.encoder_embed_dim if encoder == "t" else self.args.gnn_embed_dim
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        for head in ["projection_heads", "prediction_heads"]:
            if hasattr(self, head):
                cur_state = getattr(self, head).state_dict()
                for k, v in cur_state.items():
                    if prefix + head + "." + k not in state_dict:
                        state_dict[prefix + head + "." + k] = v
