...
class PygPCQM4MDataset(InMemoryDataset):
    def __init__(self, root='dataset', smiles2graph=smiles2graph, transform=None, pre_transform=None,
        ...
        if dataset_name is None:
            dataset_name = 'pcqm4m_kddcup2021'
            self.is_original_dataset = True
        else:
            self.is_original_dataset = False
    ...
    def download(self):
        if not self.is_original_dataset:
            # Do not download non-original dataset.
            return
